...
class PrairieClient:
    ...
    def _set_pending_error(self, exc):
        if exc is None:
            return
    ...
    def start(self, wait_s=None):
        ...
        ok = bool(self._enqueue('START', want_data=False, wait_reply=True))
        if ok:
            self._imaging = True
    ...
    def stop(self, wait_s=None):
        ...
        ok = bool(self._enqueue('STOP', want_data=False, wait_reply=True))
        if ok:
            self._imaging = False
        ...
        def _do_start():
            try:
                if self._finished:
                    return
                if not self.start():
                    raise RuntimeError('Scheduled START command failed')
            except Exception as exc:
                self._set_pending_error(exc)
        
        t = threading.Timer(float(delay_s), _do_start)
        t.daemon = True
        ...
        def _do_stop():
            try:
                if self._finished:
                    return
                if not self.stop():
                    raise RuntimeError('Scheduled STOP command failed')
            except Exception as exc:
                self._set_pending_error(exc)
        
        t = threading.Timer(float(delay_s), _do_stop)
        t.daemon = True
    ...
    def restart_after(self, stop_delay_s, start_delay_s):
        if self._finished:
            return False
        ...
        t = threading.Timer(float(stop_delay_s), _do_restart)
        t.daemon = True
