...
class PrairieServer:
    ...
    def _reader_loop(self, conn, addr):
        ...
        try:
            while self._running.is_set():
                ...
                _dump('[SERVER] TCP_RECV', line, verbose=self.verbose)
    ...
    def _ack(self, conn, data=None):
        ...
        try:
            out = (line + '\n').encode('utf-8')
            _dump('[SERVER] TCP_SEND', out, verbose=self.verbose)
            conn.sendall(out)
        except OSError:
            pass
...
def main(verbose=False):
    ...
    server = PrairieServer(local_ip=SERVER_IP, local_port=SERVER_PORT,
